...
bool Session::isSendAllowed() const
{
	return (m_cwnd > m_outstandingFrags.sum()) and (m_data_packet_count < MAX_DATA_PACKET_BURST);
}
...
void Session::updateCWND(size_t acked_bytes_this_packet, bool any_loss, bool any_naks)
{
	...
	else if(m_any_acks and (not any_naks) and (m_pre_ack_outstanding >= m_cwnd))
	{
		size_t increase = 0;
...
bool Session::onInterfaceWritable(int interfaceID, int priority)
{
	...
	if((not sentObligatoryAcks) and isSendAllowed())
	{
